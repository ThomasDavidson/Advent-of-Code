...
fn get_number_from_lines(compare: &String, compare_top: Option<&String>, compare_bottom: Option<&String>) -> i32 {
    ...
    for (pos, char) in compare.chars().enumerate(){
        ...
        // println!("pos: {}, len: {}", pos, compare.len());
        // println!("start_cmp: {:?}", start_cmp.is_some());
        ...
        if (!char.is_numeric() || pos == compare.len() - 1) && start_cmp.is_some() {
            ...
            // println!("end_val: {}, len: {}", end_val, compare.len());
